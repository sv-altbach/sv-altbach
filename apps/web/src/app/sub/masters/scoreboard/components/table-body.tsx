...
export function TableBody() {
  ...
  const notQualifiedPlayersInTop16 = playersInFinal.filter(player => player.playedTournaments < 3);
  const successorPlayers = restPlayers.filter(player => player.playedTournaments >= 3).splice(0, notQualifiedPlayersInTop16.length);
      ...
      <Table.Body>
        {playersInFinal.map((player, index) => {
          const playerRowData: PlayerRowData = {
            player: player,
            index: index,
            marker: notQualifiedPlayersInTop16.find(p => p.id === player.id) ? "not-qualified" : "none",
          }

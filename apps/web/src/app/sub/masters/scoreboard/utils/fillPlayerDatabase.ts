...
export const fillPlayerDatabase = (
  ...
  tournamentResult.data.rows.forEach(player => {
    ...
    if (playerExists) {
      playerDatabase.forEach(p => {
        if(p.id === playerExists.id) {
          p[`tournament${tournamentResult.data.tournamentNumber}`] = getMastersPoints(player, tournamentResult);
    ...
    const tournamentKey = `tournament${tournamentResult.data.tournamentNumber}` as `tournament${number}`;

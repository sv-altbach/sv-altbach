...
Bun `1.4.0` is the package manager/runtime and is pre-installed in the VM snapshot (on `PATH` via `/usr/local/bin/bun`). Dependencies are refreshed automatically on startup via `bun install`. Standard scripts are documented in `README.md` and the `package.json` files; notes below are only for non-obvious gotchas.
...
- Masters production `build` is `next build` (Node). Do not use `bun --bun next build` on Vercel: Bun 1.3.14 panics after Next 16 page generation. Local `dev` still uses `bun --bun next dev`.
...
- Lint/format uses Oxc: `bunx oxlint` and `bunx oxfmt --check` (matches CI in `.github/workflows/code-quality.yml`).

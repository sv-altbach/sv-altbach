...
- CMS admin is at `http://localhost:3003/admin`. It needs `POSTGRES_URL` and `PAYLOAD_SECRET` in `apps/cms/.env` (see `apps/cms/.env.example`). Optional local Postgres: `docker compose -f apps/cms/docker-compose.yml up -d`. `BLOB_READ_WRITE_TOKEN` is optional locally (media falls back to disk). CMS secrets stay on the CMS Vercel project only — do not put them in Club/Masters. Local CMS uses Bun: `bun --bun next` (dev uses `--webpack` — Bun+Turbopack breaks Payload SSR externals) and Payload CLI via `bunx payload` (Node; Bun runtime hits Lexical init errors). No `cross-env`/`dotenv` — Bun/Next/Payload load `.env`.
...
- Contact form needs a real `RESEND_API_KEY` in `apps/web/.env`: `apps/web/src/integrations/email.ts` throws `RESEND_API_KEY is not set` at module load, so submitting the contact form crashes without it. Club site, Masters, and CMS (with local Postgres) work without Resend; CMS still needs DB + Payload secret.

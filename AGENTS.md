...
- SVA Masters routes are served from `apps/masters` on `http://localhost:3001` (`/`, `/scoreboard`, `/finals/*`). Club → Masters links use `NEXT_PUBLIC_MASTERS_URL` (see `apps/web/.env.example`; local default `http://localhost:3001`).
